...
actor {
    ...
    public shared(msg) func deleteUser() : async Result.Result<(), Error> {
        ...
        switch(result) {
            ...
            case (?v) {
                let deletedUser = state.users.remove(callerid);
                #ok(());
            }
        };
    };

    public shared(msg) func createPost(p: PostInfo) : async Result.Result<(), Error> {
        let callerid = msg.caller;

        let user = state.users.get(callerid);
    ...
    public shared(msg) func readPost(postid: Nat) : async Result.Result<Post, Error> {
        let callerid = msg.caller;
        ...
        switch(result) {
            ...
            case (? v) {
                if (v.active == false and v.author != callerid) {
                    return #err(#NotAuthorized);
                };
                #ok(v);
            }
